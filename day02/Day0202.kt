...
fun main() {
    ...
    ranges.forEach { range ->
        ...
        sum += getSumOfRepeatingNumbersInARange(startAndEnd[0], startAndEnd[1])
        println("sum: $sum")
    }

}

private fun getSumOfRepeatingNumbersInARange(start: BigInteger, end: BigInteger): BigInteger {
    var sum: BigInteger = BigInteger.ZERO
...
private fun isRepeatingNumber(num: BigInteger): Boolean {
    val s = num.toString()
    val doubled = s + s
    val trimmed = doubled.substring(1, doubled.length - 1)
